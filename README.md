...
관심 게임이 Steam/Epic 등에서 할인하면 Discord로 알림받는 봇.
상시 서버 없이 **GitHub Actions(cron)** 로 하루 1회 실행하고, 같은 할인은 한 번만 알린다.
...
```
GitHub Actions (cron)
  → ITAD 가격 조회 (watchlist 게임 / 전체 deals)
  → data/state.json 과 비교 → 신규·개선된 할인만 필터
  → Discord Webhook 으로 embed POST
  → state.json 갱신 후 commit back
```

## 두 가지 모드 (`config.yaml` 의 `mode`)

- **`watchlist`** — 찜한 게임만 추적해 **Discord 로 직접 알림**. 조용하고 정확. `target_price`(목표가) / `min_discount`(최소 할인율) 조건 지원.
- **`deals`** — 할인 중인 게임 **전부**를 **동적 페이지**(GitHub Pages)에서 보여준다. 페이지(`docs/index.html`)가 **Cloudflare Worker** 를 통해 ITAD 데이터를 실시간으로 받아 인기순으로 렌더(검색·정렬 가능)하므로, git 에 데이터가 쌓이지 않는다. 봇은 Discord 에 **요약 1개**(현재 N개 + 인기 TOP 5(Steam 평가%) + 페이지 링크)만 보낸다.
...
4. `config.yaml` 에서 `mode: deals`.

> 페이지가 데이터를 실시간으로 받으므로 git 에 할인 목록이 커밋되지 않는다. Discord 요약을 매일 받고 싶으면 워크플로 cron 을 그대로 두고, 페이지만 쓸 거면 봇 실행 없이도 페이지는 항상 최신이다.
